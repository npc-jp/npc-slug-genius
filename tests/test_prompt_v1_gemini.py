#!/usr/bin/env python3
"""
v1.1 プロンプト動作検証スクリプト（Google Gemini版）

system-prompt-v1.txt を Gemini 2.0 Flash に投げて、30本のテストケースで
合格基準 25/30 を満たすかを測定する。

使い方:
  export GEMINI_API_KEY=AIza...
  python3 tests/test_prompt_v1_gemini.py

オプション:
  --model gemini-2.0-flash  (デフォルト)
  --temperature 0.3
  --output tests/results-v1-gemini.md
"""
...
def call_gemini(system_prompt: str, title: str, api_key: str, model: str, temperature: float) -> tuple[str, dict]:
    payload = {
        "systemInstruction": {
...
def main():
    ...
    parser.add_argument("--output", default=str(ROOT / "tests" / "results-v1-gemini.md"))
    ...
    print(f"Running {len(cases)} test cases with Gemini model={args.model}, temperature={args.temperature}")
    print("=" * 80)
    ...
    for i, case in enumerate(cases, 1):
        try:
            raw, _ = call_gemini(system_prompt, case["input"], api_key, args.model, args.temperature)
    ...
    lines = []
    lines.append(f"# プロンプト動作検証結果 v1.1 (Gemini)")
    lines.append("")
    lines.append(f"- 日時: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"- プロバイダ: Google Gemini")
